...
public abstract class MusicCommand extends Command 
{
    ...
    protected void execute(CommandEvent event) 
    {
        ...
        if(bePlaying && !((AudioHandler) Objects.requireNonNull(
                event.getGuild().getAudioManager().getSendingHandler())).isMusicPlaying(event.getJDA()))
        {
        ...
        if(beListening)
        {
            ...
            if(!userState.inVoiceChannel() || userState.isDeafened() || (current != null && ! Objects.equals(
                    userState.getChannel(), current)))
            {
